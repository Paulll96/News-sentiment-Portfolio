...
/**
 * GET /api/sentiment/top-india
 * Returns top 3 Indian (NSE/BSE) stocks by highest average sentiment score.
 * Falls back to all stocks sorted by sentiment if no Indian stocks are found.
 * Does NOT require authentication.
 */
router.get('/top-india', async (req, res) => {
    try {
        const days = parseInt(req.query.days) || 30;

        // First try: Indian stocks only (NSE/BSE or country = IN)
        const indiaResult = await query(
            `SELECT s.symbol, s.name, s.exchange, s.country,
                    AVG(ss.raw_score)::float AS wss,
                    COUNT(ss.id)::int AS article_count
             FROM stocks s
             JOIN sentiment_scores ss ON ss.stock_id = s.id
             WHERE (UPPER(s.exchange) IN ('NSE', 'BSE') OR UPPER(s.country) = 'IN')
               AND ss.analyzed_at >= NOW() - ($1 || ' days')::interval
             GROUP BY s.id, s.symbol, s.name, s.exchange, s.country
        ...
        if (indiaResult.rows.length > 0) {
            return res.json({ stocks: indiaResult.rows, source: 'india', days });
        }

        // Fallback: top stocks from ANY exchange by highest positive sentiment
        ...
        return res.json({ stocks: fallbackResult.rows, source: 'global', days });
    } catch (error) {
        console.error('top-india error:', error);
