...
async function analyzeUnprocessedArticles() {
    ...
    const trackedSymbolSet = new Set(trackedStocks.map(r => String(r.symbol || '').trim().toUpperCase()));

    // ── NEW: Load Indian NSE instruments for name-alias detection ──────────
    // We load a reasonable slice of instrument_master so that any Indian stock
    // mentioned by name or symbol in the scraped news can be auto-added to the
    // stocks table and scored — not just portfolio holdings.
    const nseInstrumentsResult = await query(
        `SELECT symbol, name, exchange, country, currency
         FROM instrument_master
         WHERE UPPER(exchange) = 'NSE'
           AND is_tradable = true
         LIMIT 5000`
    );
    const nseInstruments = nseInstrumentsResult.rows;
    // Map: SYMBOL.NS → { symbol, name, exchange, country, currency }
    const nseInstrumentMap = new Map(
        nseInstruments.map(r => [String(r.symbol || '').trim().toUpperCase(), r])
    );

    // Merge name aliases: tracked stocks + NSE instruments
    const allAliasableStocks = [
        ...trackedStocks,
        ...nseInstruments.filter(r => !trackedSymbolSet.has(String(r.symbol || '').trim().toUpperCase()))
    ];
    const nameAliases = buildNameAliases(allAliasableStocks);
    // ── END NEW ────────────────────────────────────────────────────────────
    ...
    for (const article of articles) {
        try {
            ...
            // ── NEW: Auto-add detected Indian stocks from instrument_master ────
            // For any symbol detected in this article that exists in instrument_master
            // but hasn't been added to stocks yet, insert it now so it gets scored.
            const newNseSymbols = stockSymbols.filter(
                sym => nseInstrumentMap.has(sym) && !trackedSymbolSet.has(sym)
            );
            for (const sym of newNseSymbols) {
                const inst = nseInstrumentMap.get(sym);
                try {
                    await query(
                        ...
                        [
                            sym,
                            String(inst.name || sym).trim(),
                            'Indian Equity',
                            String(inst.exchange || 'NSE').trim().toUpperCase(),
                            String(inst.country || 'IN').trim().toUpperCase(),
                            String(inst.currency || 'INR').trim().toUpperCase()
                        ]
                    );
                    trackedSymbolSet.add(sym);
                    stockMetaBySymbol.set(sym, { exchange: 'NSE' });
                    console.log(`📌 Auto-added Indian stock: ${sym} (${inst.name})`);
                } catch (_) { /* ignore duplicate race */ }
            }
            // ── END NEW ────────────────────────────────────────────────────────
